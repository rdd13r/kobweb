...
/** Method annotated with `@Api` / ApiStream objects */
...
class BackendDataProcessor(
    ...
    private val webSocketMethods = mutableListOf<ApiStreamEntry>()
    ...
    override fun finish(): BackendData {
        for (annotatedMethod in apiTargets) {
            val routeOverride = annotatedMethod.routeOverride
            ...
            val slugPrefix = if (routeOverride != null && routeOverride.startsWith("/")) {
                ...
                RouteUtils
                    .resolve(packageMappings, annotatedMethod.pkg)
            ...
            val slug = if (routeOverride != null && routeOverride.last() != '/') {
                ...
                annotatedMethod.slugFromFile
            }

            val fqn = "${annotatedMethod.pkg}.${annotatedMethod.name}"
            val route = "$slugPrefix$prefixExtra/$slug"

            when (annotatedMethod.type) {
                ApiTarget.Type.API -> apiMethods.add(ApiEntry(fqn, route))
                ApiTarget.Type.STREAM -> webSocketMethods.add(ApiStreamEntry(fqn, route))
            }
        }

        return BackendData(initMethods, apiMethods, webSocketMethods).also { it.assertValid() }
    }
}
